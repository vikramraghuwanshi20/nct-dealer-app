      ...
      inComponent:(NSInteger)component
{
    ...
    if (forPicker==0) {
        [self setdata:resultString forKey:@"Contacts"];
    }else{
        [self setdata:resultString forKey:@"Location"];
        NSLog(@"user dar %@",userData);
    }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    NSArray *titleArray=@[@"Fname",@"Lname",@"Email",@"Phone",@"Rewardnum"];
    
    [self setdata:textField.text forKey:[titleArray objectAtIndex:textField.tag-1]];
    NSLog(@"data %@",userData);
